...
function InitWebVideoCtrl() {
    ...
    if (-1 == iRet) {
        alert("您还未安装过WebComponents.exe插件！");
        return;
    }
    ...
    WebVideoCtrl.I_InitPlugin(video_div.offsetWidth, video_div.offsetHeight, {
...
// 登录
function clickLogin(tree,numTag) {

    if ("" == szIP || "" == szPort || undefined == szIP || undefined == szPort) {
        return;
    }
...
// 获取通道
function getChannelInfo(tree,numTag) {
    var szDeviceIdentify = $("#ip").val();
    ...
    WebVideoCtrl.I_GetAnalogChannelInfo(szDeviceIdentify, {
        ...
        success: function (xmlDoc) {
            ...
            $.each(oChannels, function (i) {
                ...
                channels.push({'text':name, 'tags':[id+','+false+','+szDeviceIdentify]});	//id,bZero,szDeviceIdentify
                
                //oSel.append("<option value='" + id + "' bZero='false'>" + name + "</option>");
            });
            //tree[0].nodes = channels;
            settingTree(null,tree,numTag,channels);
        ...
        success: function (xmlDoc) {
            ...
            $.each(oChannels, function (i) {
                ...
                if ("true" == $(this).find("enabled").eq(0).text()) {// 过滤禁用的零通道
                	channels.push({'text':name, 'tags':[id+','+true+','+szDeviceIdentify]});	//id,bZero,szDeviceIdentify
...
function clickStartRealPlay(iStreamType) {
    var oWndInfo = WebVideoCtrl.I_GetWindowStatus(g_iWndIndex),
        ...
        bZeroChannel = $("#channels").attr("bZero") == "true" ? true : false,
        szInfo = "";
